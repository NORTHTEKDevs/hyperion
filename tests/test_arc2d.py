...
def test_arc2d_training_baseline():
    """Baseline must hold at >= 13.5% on ARC-AGI training (55/400 = 13.75% achieved)."""
    ...
    acc = correct / total
    assert acc >= 0.135, f"training: {correct}/{total} = {acc:.4f}, expected >= 0.135"
...
def test_arc2d_evaluation_baseline():
    """Held-out evaluation must hold at >= 2.0% (currently 9/400 = 2.25%)."""
    ...
    if total == 0:
        pytest.skip("no tasks found")
    acc = correct / total
    assert acc >= 0.02, f"evaluation: {correct}/{total} = {acc:.4f}, expected >= 0.02"
