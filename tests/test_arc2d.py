...
def test_arc2d_training_baseline():
    ...
    acc = correct / total
    assert acc >= 0.08, f"training: {correct}/{total} = {acc:.4f}, expected >= 0.08"
...
def test_arc2d_evaluation_baseline():
    """Held-out evaluation must hold at >= 0.5% (currently 3/400 = 0.75%)."""
    ...
    if total == 0:
        pytest.skip("no tasks found")
    acc = correct / total
    assert acc >= 0.005, f"evaluation: {correct}/{total} = {acc:.4f}, expected >= 0.005"
